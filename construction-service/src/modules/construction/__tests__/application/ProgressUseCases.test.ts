...
describe('ProgressUseCases (Hakediş)', () => {
  ...
  it('durum makinesi: draft→submitted→approved; geçersiz geçiş reddedilir', async () => {
    ...
    const status = new ChangeProgressStatusUseCase(progress, clock, new FakeEventPublisher());
  ...
  it('submitted hakediş düzenlenemez', async () => {
    ...
    await new ChangeProgressStatusUseCase(progress, clock, new FakeEventPublisher()).execute({
      companyId: 1,
      progressId: created.id,
      status: 'submitted',
    });
