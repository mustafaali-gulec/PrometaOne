...
describe('MaterialUseCases', () => {
  ...
  it('stok hareketi (giriş/transfer/çıkış) stok cache’ini günceller', async () => {
    ...
    const rec = new RecordStockMovementUseCase(stock, materials, warehouses, new FakeEventPublisher());
    await rec.execute({
      companyId: 1,
      materialId: mat.id,
      kind: 'in',
  ...
  it('transfer için kaynak/hedef depo gerekli (tür-depo doğrulaması)', async () => {
    ...
    await assert.rejects(
      () =>
        new RecordStockMovementUseCase(stock, materials, warehouses, new FakeEventPublisher()).execute({
          companyId: 1,
          materialId: mat.id,
          kind: 'transfer',
          fromWarehouse: wA.id,
