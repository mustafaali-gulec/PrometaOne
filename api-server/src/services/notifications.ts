...
export async function generateDailyReport(companyId: number): Promise<DailyReportResult> {
  // Şirket bilgisi
  ...
  lines.push('-'.repeat(60));
  lines.push(
    `Vadesi geçen      : ${overdue.rowCount} fatura · ${fmtTL(
      overdue.rows.reduce((s, r) => s + parseFloat(r.remaining), 0),
    )} TL`,
  );
  lines.push(
    `Yaklaşan (${threshold} gün): ${dueSoon.rowCount} fatura · ${fmtTL(
  ...
  if (settings.include_upcoming_30 && upcoming30.rowCount && upcoming30.rowCount > 0) {
    ...
    lines.push('-'.repeat(60));
    lines.push(
      `Toplam ${upcoming30.rowCount} fatura · ${fmtTL(
