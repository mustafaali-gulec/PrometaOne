...
router.get(
  ...
  async (c) => {
    const cid = Number(c.req.param("cid"));
    const fy = Number(c.req.query("fiscalYear")) ||
      (await queryOne<{ fy: number }>(
        `SELECT fiscal_year AS fy FROM companies WHERE id = $1`, [cid]
      ))?.fy;
...
router.put(
  ...
  async (c) => {
    ...
    const fy = fiscalYear ?? (await queryOne<{ fy: number }>(
      `SELECT fiscal_year AS fy FROM companies WHERE id = $1`, [cid]
    ))?.fy;
    if (!fy) throw new HTTPException(400, { message: "fiscalYear belirlenemedi" });
...
router.put(
  ...
  async (c) => {
    ...
    const fy = (await queryOne<{ fy: number }>(
      `SELECT fiscal_year AS fy FROM companies WHERE id = $1`, [cid]
    ))?.fy;
    if (!fy) throw new HTTPException(400, { message: "fiscalYear belirlenemedi" });
