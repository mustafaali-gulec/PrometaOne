...
banks.get('/', async (c) => {
  const rows = await queryMany(`SELECT id, name, code, color FROM banks ORDER BY name`);
  return c.json(rows);
});
...
banks.delete('/:cid/bank-accounts/:id', requireCompanyAccess('cfo'), async (c) => {
  const cid = Number(c.req.param('cid'));
  const id = Number(c.req.param('id'));
  await execute(`UPDATE bank_accounts SET active = FALSE WHERE id = $1 AND company_id = $2`, [
    id,
    cid,
  ]);
...
transfers.post(
  ...
  async (c) => {
    ...
    await logAudit(
      c,
      'transfer_create',
      { amount: d.fromAmount, from: `${d.fromType}#${d.fromId}`, to: `${d.toType}#${d.toId}` },
      cid,
    );
    return c.json(result, 201);
  },
...
transfers.delete('/:cid/transfers/:id', requireCompanyAccess('editor'), async (c) => {
  const cid = Number(c.req.param('cid'));
  const id = Number(c.req.param('id'));
  const result = await pool.query(`DELETE FROM transfers WHERE id = $1 AND company_id = $2`, [
    id,
    cid,
  ]);
...
archives.get('/:cid/archives/:year', requireCompanyAccess('viewer'), async (c) => {
  ...
  const result = await queryOne(
    `SELECT id, fiscal_year AS "fiscalYear", fiscal_start_month AS "fiscalStartMonth",
            ...
            snapshot,
            archived_at AS "archivedAt", archived_by AS "archivedBy"
     FROM year_archives WHERE company_id = $1 AND fiscal_year = $2`,
    [cid, year],
  );
  ...
  async (c) => {
    ...
    const result = await transaction(async (client) => {
      ...
      const cats = await client.query<any>(
        `SELECT id, section, name, sort_order FROM categories WHERE company_id = $1`,
        [cid],
      );
      ...
      const closingCash =
        parseFloat(comp.rows[0].opening_cash) + totalInflow - totalOutflow - totalNonPnl;
      ...
      await client.query(
        ...
        [
          cid,
          currentFy,
          comp.rows[0].fiscal_start_month,
          comp.rows[0].opening_cash,
          closingCash,
          totalInflow,
          totalOutflow,
      ...
      if (d.clearCells) {
        await client.query(`DELETE FROM cells WHERE company_id = $1 AND fiscal_year = $2`, [
          cid,
          currentFy,
        ]);
      }
...
notifications.patch('/:cid/notification-settings', requireCompanyAccess('cfo'), async (c) => {
  const cid = Number(c.req.param('cid'));
  const body = await c.req.json();
  const map: Record<string, string> = {
    enabled: 'enabled',
  ...
  params.push(cid);
  await execute(
    `UPDATE notification_settings SET ${fields.join(', ')} WHERE company_id = $${pi}`,
    params,
  );
...
notifications.post('/:cid/notifications/generate', requireCompanyAccess('cfo'), async (c) => {
  const cid = Number(c.req.param('cid'));
  const result = await generateDailyReport(cid);
  await pool.query(
    `UPDATE notification_settings SET last_generated_at = NOW() WHERE company_id = $1`,
    [cid],
  );
...
notifications.post('/:cid/notifications/send', requireCompanyAccess('cfo'), async (c) => {
  const cid = Number(c.req.param('cid'));
