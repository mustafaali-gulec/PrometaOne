...
// ============== LIST ==============
router.get('/:cid/invoices', requireCompanyAccess('viewer'), async (c) => {
...
// ============== UPDATE ==============
router.patch('/:cid/invoices/:id', requireCompanyAccess('editor'), async (c) => {
  const cid = Number(c.req.param('cid'));
...
// ============== DELETE ==============
router.delete('/:cid/invoices/:id', requireCompanyAccess('editor'), async (c) => {
  const cid = Number(c.req.param('cid'));
  const id = Number(c.req.param('id'));
  ...
  async (c) => {
    ...
    if (data.bankAccountId && data.kasaAccountId) {
      throw new HTTPException(400, {
        message: 'Sadece banka veya kasa hesabı seçin, ikisi birden olamaz',
      });
    }
    ...
    const updated = await queryOne<any>(
      `SELECT i.*, (i.total - i.paid_amount) AS remaining FROM invoices i WHERE i.id = $1`,
      [id],
    );
    await logAudit(
      c,
      'invoice_payment',
      { invoiceId: id, amount: data.amount, date: data.date },
      cid,
    );
...
router.post('/:cid/invoices/bulk-commit', requireCompanyAccess('cfo'), async (c) => {
  const authCtx = c.get('auth');
  const cid = Number(c.req.param('cid'));

  // Fiscal year bilgisi
  const comp = await queryOne<{ fiscal_year: number }>(
    `SELECT fiscal_year FROM companies WHERE id = $1`,
    [cid],
  );
...
function formatInvoice(row: any): any {
  return {
    ...
    status:
      row.status ??
      (parseFloat(row.paid_amount) >= parseFloat(row.total) - 0.01
        ? 'paid'
