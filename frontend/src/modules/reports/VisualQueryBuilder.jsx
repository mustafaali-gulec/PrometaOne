...
const OPS = ['=', '<>', '<', '<=', '>', '>=', 'like', 'ilike', 'in', 'between', 'is null', 'is not null'];
...
export function VisualQueryBuilder({ catalog = [], value, onChange, api, lang = 'tr' }) {
  ...
  // --- mutators ---
  const addCol = () => set({ columns: [...spec.columns, { col: colOptions[0] || '', agg: '', alias: '' }] });
  const setCol = (i, patch) =>
    set({ columns: spec.columns.map((c, k) => (k === i ? { ...c, ...patch } : c)) });
  const delCol = (i) => set({ columns: spec.columns.filter((_, k) => k !== i) });

  const addFilter = () => set({ filters: [...(spec.filters || []), { col: colOptions[0] || '', op: '=', value: '' }] });
  ...
  const addOrder = () => set({ orderBy: [...(spec.orderBy || []), { col: colOptions[0] || '', dir: 'asc' }] });
  ...
  const onOpChange = (i, op) => {
    ...
    else if (op === 'in') patch.value = Array.isArray(spec.filters[i]?.value) ? spec.filters[i].value : [];
        ...
        <>
          ...
          <div className={grp()}>
            ...
            {spec.columns.map((c, i) => (
              <div key={i} className="flex items-center gap-1" style={{ flexWrap: 'wrap' }}>
                <select className="input mono" style={{ fontSize: 11, maxWidth: 180 }} value={c.col} onChange={(e) => setCol(i, { col: e.target.value })}>
                  {c.agg === 'count' && <option value="*">* (tümü)</option>}
                  {colOptions.map((o) => (
                    <option key={o} value={o}>{o}</option>
                  ))}
                </select>
                <select className="input" style={{ fontSize: 11, maxWidth: 100 }} value={c.agg || ''} onChange={(e) => setCol(i, { agg: e.target.value })}>
                  {AGGS.map((a) => (
                    <option key={a} value={a}>{a === '' ? (tr ? 'agg yok' : 'no agg') : a.toUpperCase()}</option>
                  ))}
                </select>
                <input className="input" style={{ fontSize: 11, maxWidth: 120 }} placeholder={tr ? 'takma ad' : 'alias'} value={c.alias || ''} onChange={(e) => setCol(i, { alias: e.target.value })} />
                <button className="btn" style={{ fontSize: 10, color: '#b91c1c' }} onClick={() => delCol(i)}>✕</button>
              </div>
            ))}
            {hasAgg && (
              <div className="text-xs" style={{ color: 'var(--ink-mute)' }}>
                {tr ? 'ℹ Agregasyon var → agregasız kolonlar otomatik gruplanır.' : 'ℹ Auto GROUP BY on non-aggregated columns.'}
              </div>
            )}
          </div>
          ...
          <div className={grp()}>
            ...
            {(spec.filters || []).map((f, i) => (
              <div key={i} className="flex items-center gap-1" style={{ flexWrap: 'wrap' }}>
                <select className="input mono" style={{ fontSize: 11, maxWidth: 160 }} value={f.col} onChange={(e) => setFilter(i, { col: e.target.value })}>
                  {colOptions.map((o) => (<option key={o} value={o}>{o}</option>))}
                </select>
                <select className="input" style={{ fontSize: 11, maxWidth: 110 }} value={f.op} onChange={(e) => onOpChange(i, e.target.value)}>
                  {OPS.map((o) => (<option key={o} value={o}>{o}</option>))}
                </select>
                {!NO_VALUE_OPS.has(f.op) && f.op === 'between' && (
                  <>
                    <input className="input" style={{ fontSize: 11, maxWidth: 90 }} placeholder="min" value={f.value?.[0] ?? ''} onChange={(e) => setFilter(i, { value: [e.target.value, f.value?.[1] ?? ''] })} />
                    <input className="input" style={{ fontSize: 11, maxWidth: 90 }} placeholder="max" value={f.value?.[1] ?? ''} onChange={(e) => setFilter(i, { value: [f.value?.[0] ?? '', e.target.value] })} />
                  </>
                )}
                {!NO_VALUE_OPS.has(f.op) && f.op === 'in' && (
                  <input className="input" style={{ fontSize: 11, maxWidth: 200 }} placeholder={tr ? 'a, b, c' : 'a, b, c'} value={(Array.isArray(f.value) ? f.value : []).join(', ')} onChange={(e) => setFilter(i, { value: e.target.value.split(',').map((s) => s.trim()).filter(Boolean) })} />
                )}
                {!NO_VALUE_OPS.has(f.op) && f.op !== 'between' && f.op !== 'in' && (
                  <input className="input" style={{ fontSize: 11, maxWidth: 160 }} placeholder={tr ? 'değer' : 'value'} value={typeof f.value === 'string' ? f.value : ''} onChange={(e) => setFilter(i, { value: e.target.value })} />
                )}
                <button className="btn" style={{ fontSize: 10, color: '#b91c1c' }} onClick={() => delFilter(i)}>✕</button>
              </div>
            ))}
          </div>
          ...
          <div className={grp()}>
            ...
            {(spec.orderBy || []).map((o, i) => (
              <div key={i} className="flex items-center gap-1">
                <select className="input mono" style={{ fontSize: 11, maxWidth: 180 }} value={o.col} onChange={(e) => setOrder(i, { col: e.target.value })}>
                  {colOptions.map((c) => (<option key={c} value={c}>{c}</option>))}
                </select>
                <select className="input" style={{ fontSize: 11, maxWidth: 90 }} value={o.dir} onChange={(e) => setOrder(i, { dir: e.target.value })}>
                  <option value="asc">ASC</option>
                  <option value="desc">DESC</option>
                </select>
                <button className="btn" style={{ fontSize: 10, color: '#b91c1c' }} onClick={() => delOrder(i)}>✕</button>
              </div>
            ))}
            <label className="text-xs flex items-center gap-2" style={{ color: 'var(--ink-mute)' }}>
              {tr ? 'Limit' : 'Limit'}
              <input className="input mono" type="number" style={{ fontSize: 11, maxWidth: 100 }} value={spec.limit ?? ''} onChange={(e) => set({ limit: e.target.value ? Number(e.target.value) : undefined })} />
            </label>
          </div>
          ...
          <div className="card p-2">
            ...
            {previewErr ? (
              <div className="text-xs" style={{ color: '#b91c1c' }}>⚠ {previewErr}</div>
            ) : (
              <pre className="mono" style={{ fontSize: 11, whiteSpace: 'pre-wrap', margin: 0, color: 'var(--ink)' }}>
